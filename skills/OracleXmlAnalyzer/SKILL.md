...
*   **用途**: 作為前端 LOV Service 與 Dropdown 下拉選項的實作依據。

### 2.5 DLP 前端配置參考 (Dlp Frontend Config Draft) [NEW]
*   基於邏輯預判適用的開發模式（如包含 Form 條件區、Grid 網格、<app-tab> 分頁或 Excel 匯出按鈕）。
*   提供初步的配置建議：
    *   **Form**: `*-form.control.ts` 的 `key`, `type`, `label` 等。
    *   **Grid**: `*.control.ts` 的 `headerName`, `field`, `type (DlpColDef)` 等。
    *   **Tab**: 使用 `<app-tab>` 與 `appTabTemplate` 指令。
*   **用途**: 作為開發人員在實作 `*.control.ts` 與 `*-form.control.ts` 時的快速參考。
...
## 5. DLP 前端配置參考 (Dlp Frontend Config Draft)
- 頁面類型: 標準 DLP 頁面 (包含 Form 條件區與 Excel 匯出功能)
- Form Control 配置草稿 (用於 *-form.control.ts):
  - FROM_DATE: { key: 'FROM_DATE', type: 'text', label: '年 月 份:', maxLength: 6 }
  - COMPANY_NO: { key: 'COMPANY_NO', type: 'lov', label: '公 司 別:', lovName: 'LOV_COMPANY_NO' }
```
