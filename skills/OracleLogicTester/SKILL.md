...
> 在執行診斷、模擬或腳本生成前，**必須** 先讀取以下資源範本：
> 1. [資源範本：診斷與模擬 SQL (resources/logic-sim-templates.sql)](resources/logic-sim-templates.sql)
> **未執行讀取動作即產出代碼將視為任務失敗。**

## 0. 避雷檢查清單 (Quick Checkpoints)
- [ ] **權限降級**：若讀不到 `DBA_` 表，是否已主動降級至 `ALL_` 視圖？
- [ ] **資料一致性**：測試對象與實際連線的 DB (如 `VN-SIT`) 是否一致？
- [ ] **自動編譯檢查**：部署 SP 後是否主動查詢了 `all_errors`？
- [ ] **品牌化報告**：最終報告是否標註 `使用SKILL: OracleLogicTester`？

## 目錄 (Table of Contents)
- [1. 適用情境 (Context)](#1-適用情境-context)
- [2. 深度診斷流程 (Diagnostic Stages)](#2-深度診斷流程-diagnostic-stages)
   - [階段 0：環境初始化與範本讀取](#階段-0環境初始化與範本讀取-initialization--templates)
   - [階段 0.5：結構與相依性預檢](#階段-05結構與相依性預檢-schema--dependency-pre-check)
   - [階段 1：主動診斷與權限自適應](#階段-1主動診斷與權限自適應-proactive-diagnostics)
   - [階段 2：深度溯源](#階段-2深度溯源-deep-discovery---conditional)
   - [階段 3：標準化腳本生成與執行](#階段-3標準化腳本生成與執行-auto-execution)
   - [階段 3.5：自動編譯與邏輯修正](#階段-35自動編譯與邏輯修正-auto-compile--fix)
   - [階段 4：品牌化測試報告產出](#階段-4品牌化測試報告產出-branded-final-report)
...
- [範例參考 (resources/logic-sim-templates.sql:L6)](resources/logic-sim-templates.sql#L6)

### 階段 1：主動診斷與權限自適應 (Proactive Diagnostics)
- **連線與權限**：執行腳本探測 `OADBA` 權限。若權限受限，自動降級至 `ALL_` 視圖，並在最終報告中紀錄，不中斷自動化流程。
- [範例參考 (resources/logic-sim-templates.sql:L13)](resources/logic-sim-templates.sql#L13)
...
- [範例參考 (resources/logic-sim-templates.sql:L22)](resources/logic-sim-templates.sql#L22)
